...
export const POST: RequestHandler = async ({ params, request, locals }) => {
    ...
    try {
        const groupDN = await withLdapClient(async (client) => {
            ...
            await client.modify(groupDN, [buildChange('add', 'member', members)]);
        ...
        return json({ success: true, added: members.length });
    } catch (err) {
        ...
        throw error(500, 'Failed to add one or more members. They may already belong to the group.');
    }
};
